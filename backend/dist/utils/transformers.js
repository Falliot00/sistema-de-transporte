...
exports.transformAlarmData = void 0;
...
const getAlarmStatusForFrontend = (dbStatus) => {
    ...
    if (DB_QUERY_STATUS_MAP.rejected.map(s => s.toLowerCase()).includes(lowercasedStatus))
        return 'rejected';
    if (DB_QUERY_STATUS_MAP.suspicious.map(s => s.toLowerCase()).includes(lowercasedStatus))
        return 'suspicious';
    return 'pending';
};
/**
 ...
 */
const transformAlarmData = (alarm) => ({
    id: alarm.guid,
...
exports.transformAlarmData = transformAlarmData;
