...
const getSummary = async (req, res) => {
    ...
    try {
        ...
        const [kpiData, alarmsByDay, alarmsByType, alarmsByStatus, hourlyDistribution, weeklyTrend, driverRanking, deviceSummary] = await Promise.all([
        ...
        const alarmsByTypeProcessed = alarmsByType.map(item => ({
        ...
        alarmsByStatus.forEach(item => {
            if (item.estado) {
                statusCounts[item.estado.toLowerCase()] = item._count.estado;
            }
        });
        ...
        const driverRankingProcessed = driverRanking.map(driver => {
        ...
        const topDevicesProcessed = deviceSummary.map(device => ({
